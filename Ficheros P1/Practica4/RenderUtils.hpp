...
public:
	RenderItem(physx::PxShape* _shape, const physx::PxTransform* _trans, Vector3 _color) :
		shape(_shape), transform(_trans), color(_color), references(1)
	{
		shape->acquireReference();
		RegisterRenderItem(this);
	}

	RenderItem(physx::PxShape* _shape, Vector3 _color) :
		shape(_shape), transform(NULL), color(_color), references(1)
	{
		shape->acquireReference();
		RegisterRenderItem(this);
	}
